...
```
配置完后可以查看一下
![查看npm的配置](http://static.litetools.top/blogs/hexo/0.png)
## 2.git
...
![官网亮点评论](http://static.litetools.top/blogs/hexo/1.png)
# 二、GithubPages操作
## 1.github设置
在主页这里创建一个仓库 **建议先别创建README 我们就只要一个空仓库，里面会有很多绑定仓库的提示，不会玩的话搞不好会冲突哈哈哈**
![创建github仓库](http://static.litetools.top/blogs/hexo/2.png)
## 2.服务器git配置
### 2.1配置git
...
![github密钥](http://static.litetools.top/blogs/hexo/3.png)
然后我们测试一下
`ssh -T git@github.com`
出现了用户名就是ok的，如下
![测试和git的链接](http://static.litetools.top/blogs/hexo/4.png)
### 2.2 如何关联到hexo
...
![默认分支情况](http://static.litetools.top/blogs/hexo/5.png)
后面只要`hexo d` 等传输完毕后，github这里会构建，我们可以在 github仓库页面上面的 `Actions`目录下面看到构建的进度
![actions情况](http://static.litetools.top/blogs/hexo/6.png)
**如果这里绿了就可以访问啦～**
是不是很奇怪，这里才是文章的最终结束～
# 三、hexo的启动部署
## 1.安装
...
![初始化目录](http://static.litetools.top/blogs/hexo/7.png)
**上面操作在配置了前期准备里面换源后是会比较快完成**
![当前目录文件](http://static.litetools.top/blogs/hexo/8.png)
...
![大概情况](http://static.litetools.top/blogs/hexo/9.png)
...
```
![下载主题位置](http://static.litetools.top/blogs/hexo/10.png)
...
- ![主题预览](http://static.litetools.top/blogs/hexo/10_1.png)
...
```
![预览](http://static.litetools.top/blogs/hexo/11.png)
## 4.日常更新
接下来我们就来讲讲日常的博客日志更新维护，在这之前先留意一下这里
![标签](http://static.litetools.top/blogs/hexo/12.png)
...
![改过后](http://static.litetools.top/blogs/hexo/13.png)
直接如下demo那样子修改就好了
```yaml
menu:
...
![创建项目](http://static.litetools.top/blogs/hexo/14.png)
...
我们直接点页面上很明显的
![部署](http://static.litetools.top/blogs/hexo/15.png)
...
![评论框](http://static.litetools.top/blogs/hexo/16.png)
